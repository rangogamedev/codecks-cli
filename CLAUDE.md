...
`/pm` (PM session), `/release` (version bump), `/api-ref` (command ref), `/codecks-docs <topic>` (Codecks manual), `/quality` (lint+format+mypy+pytest), `/mcp-validate` (MCP tool check), `/troubleshoot` (debug issues), `/split-features` (batch decomposition), `/doc-update` (audit docs for drift), `/changelog` (generate changelog from commits), `/docker` (Docker commands and troubleshooting), `/registry` (tag & lane registry reference)
